...
// ---------------------------------------------------------------------
// 1. FETCH SINGLE POST FROM PHP API (cached on the PHP side)
// ---------------------------------------------------------------------
async function fetchPost(slug: string) {
  const res = await fetch(
    `${process.env.NEXT_PUBLIC_BASE_URL ?? ""}/api/blog/get-post.php?slug=${slug}`,
    { next: { revalidate: 1800 } } // 30 min cache on Vercel/Next.js side
  )
...
// ---------------------------------------------------------------------
// 2. FETCH RELATED POSTS (limit 3) – we just reuse the “all” endpoint
// ---------------------------------------------------------------------
async function fetchRelatedPosts(currentSlug: string, limit = 3) {
  const res = await fetch(
    `${process.env.NEXT_PUBLIC_BASE_URL ?? ""}/api/blog/get-all.php?page=1`,
...
// ---------------------------------------------------------------------
// 3. PAGE COMPONENT (Server Component – runs on the server)
// ---------------------------------------------------------------------
interface BlogPostPageProps {
  params: { slug: string }
}
