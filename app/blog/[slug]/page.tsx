...
async function fetchRelatedPosts(currentSlug: string, limit = 3) {
  const res = await fetch(
    `http://localhost/photography-portfolio-backend/posts.php?page=1`,
    { next: { revalidate: 1800 } }
  )
  if (!res.ok) return []
  ...
  return data?.posts.filter((p: any) => p.slug !== currentSlug).slice(0, limit)
}
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {relatedPosts.map((p: any) => (
              <Link key={p.id} href={`/blog/${p.slug}`}>
                <article className="group bg-card border rounded-xl overflow-hidden">
                  <div className="aspect-[16/10] overflow-hidden">
                    <img
                  ...
                  <div className="p-5">
                    <span className="text-primary text-xs uppercase">{p.category}</span>
                    <h3 className="text-base font-semibold mt-2 line-clamp-2">{p.title}</h3>
                    <div className="flex items-center gap-2 mt-3 text-xs text-muted-foreground">
                      <Clock className="h-3 w-3" />
                      {p.read_time}
                    </div>
